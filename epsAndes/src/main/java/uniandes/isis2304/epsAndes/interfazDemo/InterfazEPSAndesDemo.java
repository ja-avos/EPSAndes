 ...
 * 
 * @author Germán Bravo Modificado por: Juan Avelino
 */
...
//			e.printStackTrace ();
	 ...
	 *****************************************************************/
    /**
     * Demostración de creación, consulta y borrado de Rol 
    ...
    public void demoRol( )
    {
    	try 
    	{
			...
			VORol rol = eps.addRol(nombreRol);
			if (rol == null)
			{
				rol = eps.getRolByNombre(nombreRol);
				errorTipoBebida = true;
			}
			List <Rol> lista = eps.darRoles();
			long tbEliminados = eps.eliminarRolPorId(rol.getId_Rol());
...
//			e.printStackTrace();
    ...
    public void demoTipoID( )
    {
    	try 
    	{
    		// Ejecución de la demo y recolección de los resultados
			// ATENCIÓN: En una aplicación real, los datos JAMÁS están en el código
			String nombreTipo = "Cedula";
			boolean errorTipo = false;
			VOTipoID tipo = eps.addTipoID(nombreTipo);
			if (tipo== null)
			{
				tipo = eps.getTipoIDByNombre(nombreTipo);
				errorTipo = true;
			}
			List <TipoID> lista = eps.darTiposID();
			long tbEliminados = eps.eliminarTipoIDPorId(tipo.getId_Tipo());
			
			// Generación de la cadena de caracteres con la traza de la ejecución de la demo
			String resultado = "Demo de creación y listado de TipoID\n\n";
			resultado += "\n\n************ Generando datos de prueba ************ \n";
			if (errorTipo)
			{
			...
			resultado +=  "\n" + listarTiposID (lista);
			resultado += "\n\n************ Limpiando la base de datos ************ \n";
			resultado += tbEliminados + " TiposID eliminados\n";
			resultado += "\n Demo terminada";
...
//			e.printStackTrace();
	 ...
	 *****************************************************************/
    /**
     * Genera una cadena de caracteres con la lista de los roles recibida: una línea por cada rol
     * @param lista - La lista con los roles
     * @return La cadena con una líea para cada rol recibido
     */
    private String listarRoles(List<Rol> lista) 
    {
    ...
    private String listarTiposID(List<TipoID> lista) 
    {
    	String resp = "Los tipos ID existentes son:\n";
    	int i = 1;
        for (VOTipoID tb : lista)
        {
        	resp += i++ + ". " + tb.toString() + "\n";
        }
        return resp;
